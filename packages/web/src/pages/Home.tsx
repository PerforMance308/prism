    ...
    <div className="h-full bg-surface-container flex flex-col">
      ...
      <div className="shrink-0 pb-6">
        <div className="max-w-3xl mx-auto px-6 w-full">
          ...
          {/* Recent sessions — only when no messages */}
          {!hasMessages && sessions.length > 0 && (
            <motion.section
              className="mt-8 w-full"
              variants={fadeIn}
              initial="hidden"
              ...
              <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-thin">
                {sessions.map((session) => (
                  <button
                    ...
                    type="button"
                    onClick={() => {
                      void globalChat.loadSession(session.id);
                    }}
                    className="shrink-0 w-52 text-left bg-surface-low hover:bg-surface-high rounded-xl p-3.5 cursor-pointer transition-all duration-200"
                  >
                    <div className="flex items-center gap-2 mb-2">
